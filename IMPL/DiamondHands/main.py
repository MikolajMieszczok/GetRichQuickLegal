...
def write_data_into_column(worksheet_temp, arr, column):
    ...
    for i, data in enumerate(arr):
        worksheet_temp.write(str(column) + str(i+1), str(data))
...
for x in range(len(year_bought)):
    if year_bought[x] == "Today":
        todays_date.append(str(datetime.now().strftime("%Y-%m-%d")))
for x in range(len(todays_date)):
    sixty_days_ago.append(str((datetime.now() + timedelta(days=-60)).strftime("%Y-%m-%d")))
res = trim_array(res, len(todays_date))
stocks = trim_array(stocks, len(todays_date))
filed_after = trim_array(filed_after, len(todays_date))
#Transfering Data to EXCEL
workbook = xlsxwriter.Workbook("C:/Users/Mikołaj/Desktop/Enginer/FIRSTEXCEL.xlsx")
...
write_data_into_column(worksheet, sixty_days_ago, "D")
write_data_into_column(worksheet, todays_date, "E")
workbook.close()