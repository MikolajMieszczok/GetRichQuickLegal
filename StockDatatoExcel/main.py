...
for row in range(1, 10):
    try:
        print(row)
        row = str(row)
        not_filtered = yf.Ticker(str(ws["B" + row].value).split(".")[0]).history(start = str(((ws["D" + row]).value)).split(" ")[0], end = str(((ws["E" + row]).value)).split(" ")[0], interval = "1d")
        #print(not_filtered)
...
for x in range(1, max_row):
    ...
    print(key_p)
    division : float = ws_second["E" + str(x)].value
    if key_p != None:
        ...
        for y in range(len(columns)-1):
            if ws_second[str(columns[y + 1] + str(x))].value is not None:
